...
- 把结构化叙事计划扩展成故事设计文本。
- 明确课堂游戏的叙事主线、情绪推进、角色使用方式和教学意图。

输出：

- `jobs/{job_id}/state/story_design.txt`

### 5.4 Game Structure Design
...
5. 打开 `webgal_backend/prompts.py`，理解参数如何影响生成内容。
...
```mermaid
graph TD
    phase0[phase0:归途] --> phase1[phase1:周旋]
    phase1 -->|敷衍苏文纨| phase6[phase6:苏文纨的报复]
    phase1 -->|追求唐晓芙| phase2[phase2:裂痕]
    phase6 --> phase2
    phase2 --> phase7[phase7:唐晓芙的信]
    phase7 --> phase3[phase3:远行]
    phase2 --> phase3
    phase3 --> phase4[phase4:笼中]
    phase4 --> phase5[phase5:城门]
    phase5 -->|离开| true_ending[true_ending]
    phase5 -->|回头| normal_ending[normal_ending]
    phase7 -.->|隐藏条件| hidden_ending[hidden_ending]
    phase5 -.-> hidden_ending
```
